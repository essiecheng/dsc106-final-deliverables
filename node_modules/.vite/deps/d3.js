import {
  ...
  circle_default2 as circle_default3,
  clipRectangle,
  conicConformalRaw,
  ...
  *neighbors(i) {
  ...
  _project(x0, y0, vx, vy) {
    ...
    if (vy < 0) {
      if (y0 <= this.ymin)
        return null;
      if ((c5 = (this.ymin - y0) / vy) < t)
        y3 = this.ymin, x3 = x0 + (t = c5) * vx;
    } else if (vy > 0) {
      if (y0 >= this.ymax)
        return null;
      if ((c5 = (this.ymax - y0) / vy) < t)
        y3 = this.ymax, x3 = x0 + (t = c5) * vx;
    }
    if (vx > 0) {
      if (x0 >= this.xmax)
        return null;
      if ((c5 = (this.xmax - x0) / vx) < t)
        x3 = this.xmax, y3 = y0 + (t = c5) * vy;
    } else if (vx < 0) {
      if (x0 <= this.xmin)
        return null;
      if ((c5 = (this.xmin - x0) / vx) < t)
        x3 = this.xmin, y3 = y0 + (t = c5) * vy;
    }
    return [x3, y3];
  }
  _edgecode(x3, y3) {
...
function link_default(links) {
  var id2 = index2, strength = defaultStrength, strengths, distance = constant_default6(30), distances, nodes, count3, bias, random, iterations = 1;
  if (links == null)
    links = [];
...
function encloseBasis3(a3, b, c5) {
  var x1 = a3.x, y1 = a3.y, r1 = a3.r, x22 = b.x, y22 = b.y, r2 = b.r, x3 = c5.x, y3 = c5.y, r3 = c5.r, a22 = x1 - x22, a32 = x1 - x3, b22 = y1 - y22, b32 = y1 - y3, c22 = r2 - r1, c32 = r3 - r1, d1 = x1 * x1 + y1 * y1 - r1 * r1, d2 = d1 - x22 * x22 - y22 * y22 + r2 * r2, d3 = d1 - x3 * x3 - y3 * y3 + r3 * r3, ab4 = a32 * b22 - a22 * b32, xa = (b22 * d3 - b32 * d2) / (ab4 * 2) - x1, xb = (b32 * c22 - b22 * c32) / ab4, ya = (a32 * d2 - a22 * d3) / (ab4 * 2) - y1, yb = (a22 * c32 - a32 * c22) / ab4, A = xb * xb + yb * yb - 1, B2 = 2 * (r1 + xa * xb + ya * yb), C = xa * xa + ya * ya - r1 * r1, r = -(Math.abs(A) > 1e-6 ? (B2 + Math.sqrt(B2 * B2 - 4 * A * C)) / (2 * A) : C / B2);
  return {
    x: x1 + xa + xb * r,
    y: y1 + ya + yb * r,
    r
...
function zoom_default2() {
  ...
  zoom.scaleTo = function(selection2, k, p, event) {
    zoom.transform(selection2, function() {
      ...
      return constrain(translate(scale2(t0, k1), p0, p1), e, translateExtent);
    }, p, event);
  };
  ...
  function dblclicked(event, ...args) {
    if (!filter3.apply(this, arguments))
      return;
    var t0 = this.__zoom, p0 = pointer_default(event.changedTouches ? event.changedTouches[0] : event, this), p1 = t0.invert(p0), k1 = t0.k * (event.shiftKey ? 0.5 : 2), t1 = constrain(translate(scale2(t0, k1), p0, p1), extent2.apply(this, args), translateExtent);
    noevent_default3(event);
    if (duration > 0)
